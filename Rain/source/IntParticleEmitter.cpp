#include "headers/IntParticleEmitter.h"
...
IntParticleEmitter::IntParticleEmitter(Drawable* _model, int number) {
    ...
    counter.resize(number_of_particles, 0.0f);
...
glm::vec4 IntParticleEmitter::calculateBillboardRotationMatrix(glm::vec3 particle_pos, glm::vec3 camera_pos)
{
    ...
    dir.y = 0; // in case you want only yaw rotations
...
void IntParticleEmitter::bindAndUpdateBuffers()
{
    ...
    for (int i = 0; i < p_attributes.size(); i++) {
        ...
        translations[i] = glm::translate(glm::mat4(), p.x);
    }
    ...
    glBindBuffer(GL_ARRAY_BUFFER, translations_buffer);
    glBufferData(GL_ARRAY_BUFFER, number_of_particles * sizeof(glm::mat4), NULL, GL_STREAM_DRAW); // Buffer orphaning and reallocating to avoid synchronization,
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, number_of_particles * sizeof(glm::mat4), &rotations[0]);

    glBindBuffer(GL_ARRAY_BUFFER, scales_buffer);
    glBufferData(GL_ARRAY_BUFFER, number_of_particles * sizeof(float), NULL, GL_STREAM_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, number_of_particles * sizeof(float), &scales[0]);
}
...
void IntParticleEmitter::configureVAO()
{
    ...
    glGenBuffers(1, &translations_buffer);
    glBindBuffer(GL_ARRAY_BUFFER, translations_buffer);
